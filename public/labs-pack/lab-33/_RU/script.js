...
function updateExpression(totalObjects, groupSize) {
    ...
    if (remainder === 0) {
        ...
        expressionSpan.textContent = `${totalObjects} ÷ ${groupSize} = ${groups} (остаток ${remainder})`;
    }
}
...
function updateExplanation(totalObjects, groupSize) {
    ...
    if (remainder === 0) {
        explanationP.textContent = `Разделим ${totalObjects} объектов на группы по ${groupSize}. Получим ${groups} равных групп.`;
    } else {
        explanationP.textContent = `Разделим ${totalObjects} объектов на группы по ${groupSize}. Получим ${groups} полных групп и ${remainder} объекта останется.`;
    }
}
...
function performDivision() {
    ...
    if (groupSize > totalObjects) {
        alert('Размер группы не может быть больше общего количества объектов!');
        return;
    }
...
function createGroups() {
    ...
    if (groupCount <= 0 || groupCount > 15) {
        alert('Пожалуйста, введите количество групп от 1 до 15!');
        return;
    }
    ...
    for (let i = 0; i < groupCount; i++) {
        ...
        group.textContent = 'Перетащите объекты сюда';
...
function updateGroups() {
    ...
    if (newGroupCount > currentGroupCount) {
        ...
        if (removedCount < groupsToRemove) {
            alert(`Не удалось удалить ${groupsToRemove} групп, так как они содержат объекты. Сначала переместите объекты в другие группы.`);
        }
    }
...
function generateRecommendations(totalObjects, targetGroups, analysis) {
    ...
    if (analysis.totalGroups < targetGroups) {
        recommendations.push('Создайте дополнительные группы для достижения цели');
    }
    
    if (analysis.totalGroups > targetGroups) {
        recommendations.push('Удалите лишние группы или объедините их');
    }
    
    if (!analysis.isEqualGroups) {
        recommendations.push('Переместите объекты между группами для создания равных групп');
        
        if (analysis.sizeVariation > 2) {
            recommendations.push('Попробуйте распределить объекты более равномерно');
        }
    }
    
    const expectedSize = Math.floor(totalObjects / targetGroups);
    if (analysis.averageGroupSize !== expectedSize) {
        recommendations.push(`Стремитесь к ${expectedSize} объектам в каждой группе`);
    }
    
    if (analysis.isEqualGroups && analysis.totalGroups === targetGroups) {
        recommendations.push('Отличная работа! Группы созданы правильно');
    }
...
function displayRecommendations(recommendations) {
    ...
    if (recommendations.length === 0) {
        html += '<p>Продолжайте в том же духе!</p>';
    } else {
