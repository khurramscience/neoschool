...
function showDraw() {
  ...
  btnSubmit.classList.add('hidden');
  if (btnGlueTool) btnGlueTool.classList.add('hidden');
  // restore draw board (ensure default color)
  if (!cellsDraw || cellsDraw.length === 0) {
...
function nextDeepTask() {
  if (deepRound.current >= deepRound.total) {
    ...
    addTeacherMsg('Want another round? Click Deep again or press New set.');
    return;
  }
  deepRound.current += 1;
