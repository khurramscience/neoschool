class AdditionLab {
    ...
    displayProblem() {
        ...
        html += '<button id="showCarryBtn" class="btn-info" style="font-size: 0.9rem; padding: 8px 16px;">👁️ Show carryovers</button>';
        html += '</div>';
    ...
    checkAnswer() {
        ...
        if (userAnswer === correctAnswer) {
            this.stats.correct++;
            this.showResult('Correct! 🎉 Excellent work!', 'success');
            this.highlightCorrectAnswer();
        } else {
            this.showResult(`Incorrect. Correct answer: ${correctAnswer}`, 'error');
    ...
    showSolution() {
        ...
        html += '<h4 style="margin-bottom: 15px; color: #333;">📝 Step-by-step explanation:</h4>';
        
        steps.forEach((step, index) => {
            const positionName = ['ones', 'tens', 'hundreds', 'thousands', 'ten thousands'][step.position] || `${step.position + 1}th position`;
            
            html += `<div style="margin-bottom: 15px; padding: 10px; border-left: 3px solid #667eea; background: white;">`;
            html += `<strong>Step ${index + 1}: Adding ${positionName}</strong><br>`;
            
            if (step.carry > 0) {
                html += `<span style="color: #e74c3c;">➕ Carry from previous position: ${step.carry}</span><br>`;
            }
            ...
            html += `Write: <span style="font-weight: bold;">${step.remainder}</span>`;
            
            if (step.carry > 0 && step.position < maxLength - 1) {
                html += `, carry: <span style="color: #e74c3c; font-weight: bold;">${step.carry}</span> to next position`;
            }
        ...
        html += `<strong>🎯 Final result: ${this.currentProblem.sum}</strong>`;
        html += '</div>';
        html += '</div>';
    ...
    toggleCarryDisplay() {
        ...
        if (carryRow.style.opacity === '1') {
            ...
            showCarryBtn.textContent = '👁️ Show carryovers';
            ...
            showCarryBtn.textContent = '🙈 Hide carryovers';
