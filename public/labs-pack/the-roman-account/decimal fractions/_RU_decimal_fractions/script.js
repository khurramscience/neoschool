...
function initializeVisualization() {
    ...
    visualizeBtn.addEventListener('click', function() {
        ...
        if (decimal >= 0 && decimal <= 1) {
            ...
            alert('Пожалуйста, введите число от 0 до 1');
        }
    });
    ...
    // Показать начальную визуализацию
...
function decimalToFraction(decimal) {
    // Простые случаи для лучшего понимания
    ...
    // Округлить до 3 знаков после запятой для сравнения
    ...
    // Если нет простой дроби, показать десятичную
...
function checkFractionConversion() {
    ...
    if (isNaN(numerator) || isNaN(denominator) || isNaN(userResult)) {
        showFeedback(feedback, 'Пожалуйста, заполните все поля числами', 'incorrect');
        return;
    }
    
    if (denominator === 0) {
        showFeedback(feedback, 'Знаменатель не может быть равен нулю!', 'incorrect');
        return;
    }
    ...
    if (Math.abs(userResult - correctResult) < tolerance) {
        showFeedback(feedback, `Правильно! ${numerator}/${denominator} = ${correctResult.toFixed(3)}`, 'correct');
    } else {
        showFeedback(feedback, `Неправильно. ${numerator}/${denominator} = ${correctResult.toFixed(3)}`, 'incorrect');
    }
}
...
function checkComparison() {
    ...
    if (isNaN(num1) || isNaN(num2)) {
        showFeedback(feedback, 'Пожалуйста, введите числа', 'incorrect');
        return;
    }
    ...
    if (isCorrect) {
        showFeedback(feedback, `Правильно! ${num1} ${operator} ${num2}`, 'correct');
    } else {
        showFeedback(feedback, `Неправильно. Правильный ответ: ${num1} ${correctOperator} ${num2}`, 'incorrect');
    }
}
...
function generateNewQuestion() {
    ...
    // Скрыть кнопку "Следующий вопрос"
    nextQuestionBtn.style.display = 'none';
    
    // Генерировать целевую дробь в зависимости от уровня
    let targetDecimal;
    if (currentLevel <= 3) {
        // Простые дроби для начальных уровней
        ...
        // Случайные дроби с тремя знаками
...
function generateOptions(targetDecimal) {
    ...
    // Генерировать неправильные варианты
    while (options.length < 4) {
        let wrongOption;
        if (currentLevel <= 3) {
            // Простые неправильные варианты
            wrongOption = Math.round((Math.random() * 0.9 + 0.1) * 100) / 100;
        } else {
            // Более сложные неправильные варианты
            wrongOption = Math.round((Math.random() * 0.999) * 1000) / 1000;
        }
        
        // Убедиться, что вариант уникален
...
function checkAnswer(selectedOption, targetDecimal, button) {
    ...
    if (isCorrect) {
        ...
        showFeedback(document.getElementById('comparison-feedback'), 'Правильно! +' + (currentLevel * 10) + ' очков', 'correct');
        
        // Увеличить уровень каждые 3 правильных ответа
        if (currentScore % (currentLevel * 30) === 0) {
            currentLevel++;
            showFeedback(document.getElementById('comparison-feedback'), `Уровень повышен! Теперь уровень ${currentLevel}`, 'correct');
        }
    } else {
        button.classList.add('incorrect');
        showFeedback(document.getElementById('comparison-feedback'), 'Неправильно! Попробуйте еще раз', 'incorrect');
    }
