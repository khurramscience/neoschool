...
(function initPractice() {
  ...
  function rotatePiece(id) {
    ...
    if (ok) {
      ...
      setResult('Повернули треугольник.', '');
    } else {
      // вернуть старые клетки
      occupy(p.cells, true);
      setResult('Не удаётся повернуть: нет места.', 'warn');
    }
  }
      ...
      try {
        ...
        if (data.moveId) {
          ...
          if (best && canPlace(p.kind, p.side, p.rot || 0, best.x, best.y)) {
            ...
            setResult('Переместили фигуру.', '');
          } else {
            // вернуть
            occupy(p.cells, true);
            setResult('Нельзя переместить в эту область.', 'warn');
          }
        } else {
          ...
          else setResult('Сюда фигуру поставить нельзя (нет подходящего свободного места).', 'warn');
        }
      } catch {}
    });
  }
  ...
  function refreshSum() {
    ...
    if (currentSum === target) {
      setResult('Отлично! Сумма совпала с целевым квадратом.', 'ok');
    } else if (currentSum < target) {
      setResult(`Ещё нужно добавить ${target - currentSum} точек.`, 'warn');
    } else {
      setResult(`Лишних ${currentSum - target} точек. Удалите одну или несколько фигур.`, 'err');
    }
  }
  ...
  function showHint() {
    const n = tasks[idx].n;
    const n2 = S(n);
    const basic = `Подсказка: используйте равенство T(${n}) + T(${n - 1}) = ${n}²`;
    const hint = coinHint(n2);
    if (hint.ok && hint.parts.length) {
      const list = hint.parts
        .map(p => `${p.count}×${p.kind === 'sq' ? 'S' : 'T'}(${p.side})`)
        .join(' + ');
      setResult(`${basic}. Возможное разложение: ${list}.`, 'warn');
    } else {
      setResult(`${basic}. Попробуйте комбинировать фигуры из палитры.`, 'warn');
    }
  }
  ...
  function renderTask() {
    ...
    clearWorkspace();
    setResult('Перетащите фигуры в рабочую область и составьте точное количество точек.', '');
  }
  ...
  btnCheck.addEventListener('click', () => {
    if (currentSum === target) {
      setResult('Верно! Задание засчитано.', 'ok');
      solvedMask[idx] = true;
      persist();
    } else if (currentSum < target) {
      setResult(`Недобор: нужно ещё ${target - currentSum} точек.`, 'warn');
    } else {
      setResult(`Перебор на ${currentSum - target} точек.`, 'err');
    }
  });
