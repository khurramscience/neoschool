class MathGame {
        ...
        if (this.dragDirection === 'horizontal') {
            ...
            if (col === lastCell.col && Math.abs(row - lastCell.row) === 1) {
                ...
                if (col !== secondLastCell.col) {
                    console.log(`🔄 Поворот с горизонтального на вертикальный: ${row},${col}`);
                    this.dragDirection = 'vertical';
                    return true;
                }
            }
            ...
            if (row === lastCell.row && Math.abs(col - lastCell.col) === 1) {
                ...
                if (row !== secondLastCell.row) {
                    console.log(`🔄 Поворот с вертикального на горизонтальный: ${row},${col}`);
                    this.dragDirection = 'horizontal';
                    return true;
                }
            }
    ...
    canAddCell(row, col) {
        ...
        if (this.selectedCells.some(cell => cell.row === row && cell.col === col)) {
            console.log(`❌ Ячейка ${row},${col} уже выбрана`);
            return false;
        }
        
        // Если это первая ячейка, можно выбрать любую
        if (this.selectedCells.length === 0) {
            console.log(`✅ Первая ячейка: ${row},${col}`);
            return true;
        }
        ...
        if (this.selectedCells.length === 1) {
            ...
            console.log(`🎯 Определено направление: ${this.dragDirection}`);
        }
        ...
        if (this.selectedCells.length >= 2) {
            ...
            if (canTurn) {
                console.log(`🔄 Поворот линии на ${row},${col}`);
                return true;
            }
        }
        ...
        if (this.dragDirection === 'horizontal') {
            ...
            if (canAdd) {
                console.log(`➡️ Горизонтальное продолжение: ${row},${col}`);
            } else {
                console.log(`❌ Нельзя добавить горизонтально: ${row},${col}`);
            }
            return canAdd;
        } else if (this.dragDirection === 'vertical') {
            ...
            if (canAdd) {
                console.log(`⬇️ Вертикальное продолжение: ${row},${col}`);
            } else {
                console.log(`❌ Нельзя добавить вертикально: ${row},${col}`);
            }
            return canAdd;
        }
        
        console.log(`❌ Неизвестное направление для ${row},${col}`);
        return false;
    }
    ...
    selectCell(row, col) {
        ...
        if (cell && !cell.classList.contains('selected')) {
            ...
            console.log(`✅ Выбрана ячейка: ${this.gameBoard[row][col]} (${row},${col})`);
            console.log(`📊 Всего выбрано: ${this.selectedCells.length} ячеек`);
            console.log(`🎯 Направление: ${this.dragDirection}`);
        }
    }
    ...
    checkSequence() {
        ...
        if (!this.isValidSequence()) {
            console.log('❌ Последовательность не является валидной');
            this.clearSelection();
            return;
        }
        ...
        // Отладочная информация
        console.log('Проверка последовательности:', {
    ...
    showLevelUpNotification() {
        ...
        `;
        notification.textContent = `🎉 Уровень ${this.level}!`;
    ...
    findPossibleSequences() {
        console.log('🔍 Поиск возможных последовательностей...');
        let foundSequences = [];
        ...
        console.log(`📊 Найдено ${foundSequences.length} последовательностей:`);
        foundSequences.forEach((seq, index) => {
            console.log(`${index + 1}. ${seq.type} (${seq.length} ячеек): ${seq.sequence.map(c => c.value).join(' + ')} = ${seq.sum}`);
        });
        ...
        if (foundSequences.length > 0) {
            ...
            console.log(`🏆 Самая длинная последовательность: ${longest.length} ячеек (${longest.type})`);
        }
