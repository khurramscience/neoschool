class MathMosaicGame {
    ...
    renderGrid() {
        ...
        gameGrid.style.setProperty('--cell-size', `${this.getCellSize()}px`);
        gameGrid.style.setProperty('--grid-size', this.gridSize);
        // Для выравнивания сумм
        const topSums = document.querySelector('.top-sums');
        if (topSums) topSums.style.setProperty('--grid-size', this.gridSize);
    ...
    showResult(isCorrect, isComplete) {
        ...
        if (isCorrect && isComplete) {
            ...
            message.textContent = `You successfully completed level ${this.currentLevel}! All sums match.`;
            nextBtn.style.display = 'inline-block';
        } else if (isComplete) {
            title.textContent = '❌ Try Again';
            message.textContent = 'Not all sums match. Check your calculations and try again.';
            nextBtn.style.display = 'none';
        } else {
            title.textContent = '⚠️ Grid Not Complete';
            message.textContent = 'Fill all cells to check the solution.';
            nextBtn.style.display = 'none';
        }
