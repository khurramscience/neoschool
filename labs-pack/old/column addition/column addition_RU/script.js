...
// Функции для интерактивной демонстрации
    ...
    if (step === 0) {
        demoArea.innerHTML = `
            <div style="text-align: center;">
                ...
                <p>Нажми "Следующий шаг" чтобы увидеть решение</p>
            </div>
        `;
    } else if (step === 1) {
        // Показываем сложение единиц
        ...
        demoArea.innerHTML = `
            <div style="text-align: center;">
                <h4>Шаг 1: Складываем единицы</h4>
                <div style="font-family: 'Courier New', monospace; font-size: 1.2rem; margin: 20px 0;">
                    <div>${num1}</div>
                    <div>+ ${num2}</div>
                ...
                <p>${units1} + ${units2} = ${unitsSum}. Записываем ${unitsResult}${carry > 0 ? `, ${carry} переносим` : ''}</p>
            </div>
        `;
    } else if (step === 2) {
        // Показываем сложение десятков
        ...
        demoArea.innerHTML = `
            <div style="text-align: center;">
                <h4>Шаг 2: Складываем десятки</h4>
                <div style="font-family: 'Courier New', monospace; font-size: 1.2rem; margin: 20px 0;">
                    <div>${num1}</div>
                    <div>+ ${num2}</div>
                    <div>+ ${carry1} (перенос)</div>
                    <div style="border-top: 2px solid white; margin: 10px 0; padding-top: 10px;">
                        ${carry2 > 0 ? carry2 : ' '} ${tensResult} ${unitsResult}
                    </div>
                </div>
                <p>${tens1} + ${tens2} + ${carry1} = ${tensSum}. Записываем ${tensResult}${carry2 > 0 ? `, ${carry2} переносим` : ''}</p>
            </div>
        `;
    } else if (step === 3) {
...
function showStep() {
    if (currentDemo) {
        ...
        alert('Сначала сгенерируй пример!');
    }
}

function resetDemo() {
    currentDemo = null;
    document.getElementById('demo-area').innerHTML = '<p>Нажми "Новый пример" чтобы начать демонстрацию</p>';
}
...
function checkAnswer() {
    ...
    if (isNaN(userAnswer)) {
        feedback.innerHTML = '<p style="color: #721c24; background: #f8d7da; padding: 10px; border-radius: 5px;">Пожалуйста, введи число!</p>';
        return;
    }
    ...
    if (userAnswer === currentPractice.result) {
        ...
        feedback.innerHTML = '<p style="color: #155724; background: #d4edda; padding: 10px; border-radius: 5px;">✅ Правильно! Молодец!</p>';
    } else {
        feedback.innerHTML = `<p style="color: #721c24; background: #f8d7da; padding: 10px; border-radius: 5px;">❌ Неправильно. Правильный ответ: ${currentPractice.result}</p>`;
    }
...
function showSolution() {
    ...
    feedback.innerHTML = `
        <div style="background: #e3f2fd; padding: 15px; border-radius: 10px; margin-top: 15px;">
            ...
            <p style="margin-top: 10px;"><strong>Проверка:</strong> ${num1} + ${num2} = ${result}</p>
        </div>
    `;
}
...
function submitTestAnswer() {
    ...
    if (isCorrect) {
        feedback.innerHTML = '<p style="color: #155724; background: #d4edda; padding: 10px; border-radius: 5px;">✅ Правильно!</p>';
    } else {
        feedback.innerHTML = `<p style="color: #721c24; background: #f8d7da; padding: 10px; border-radius: 5px;">❌ Неправильно. Правильный ответ: ${question.result}</p>`;
    }
...
function showTestResults() {
    ...
    document.getElementById('test-score').innerHTML = `
        ...
        <p style="font-size: 1.2rem; margin-bottom: 10px;">Правильных ответов: ${correctAnswers} из 10</p>
        <p style="font-size: 1.2rem; margin-bottom: 20px;">Процент выполнения: ${percentage}%</p>
    `;
    
    // Показываем детали ответов
    let detailsHTML = '<h4 style="margin-bottom: 15px;">Детали ответов:</h4>';
    testAnswers.forEach((answer, index) => {
        ...
        detailsHTML += `
            <div style="margin-bottom: 10px; padding: 10px; background: ${answer.isCorrect ? '#d4edda' : '#f8d7da'}; border-radius: 5px;">
                <strong>Вопрос ${index + 1}:</strong> ${answer.question.num1} + ${answer.question.num2} = ${answer.question.result}
                <br>Твой ответ: ${answer.userAnswer} ${status}
            </div>
        `;
    });
