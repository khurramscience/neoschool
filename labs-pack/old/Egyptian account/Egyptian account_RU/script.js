...
function initPractice() {
    ...
    document.getElementById('check-btn').addEventListener('click', () => {
        ...
        if (builderSum === currentTaskNumber) {
            ...
            feedback.textContent = `❌ Неправильно. Вы получили ${builderSum}, а нужно было ${currentTaskNumber}`;
...
function initQuiz() {
    const questions = [
        ...
        {
            question: "Сколько будет 2 × 1000 + 3 × 100 + 4 × 10 + 5 × 1?",
    ...
    function showQuestion() {
        ...
        question.options.forEach((option, index) => {
            ...
            // Если вопрос про иероглиф или запись числа
            if (
                question.question.includes('иероглиф') ||
                question.question.includes('обозначает число') ||
                question.question.includes('записать число')
            ) {
